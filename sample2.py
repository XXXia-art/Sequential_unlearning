...
def main():
    ...
    parser.add_argument('--data_csv', type=str, default='', help='Custom CSV data path (overrides default)')
    args = parser.parse_args()

    bs = args.batch_size
    mode_list = args.mode.replace(' ', '').split(',')
    content_list = split_contents_arg(args.contents)
    ...
    if 'edit' in mode_list:
        ...
        if args.edit_ckpt.endswith(".safetensors"):
            state_dict = load_safetensors(args.edit_ckpt, device="cpu")
        else:
            state_dict = torch.load(args.edit_ckpt, map_location='cpu')
    ...
    # Sampling process
    if args.data_csv:
        dataset = AdaDataset(data_path=args.data_csv, guidance_scale=args.guidance_scale)
    elif content_list == ['nudity']:
        dataset = AdaDataset(data_path='data/i2p_benchmark.csv')
    elif content_list == ['coco']:
        dataset = AdaDataset(data_path ='data/mscoco.csv', seed=args.seed, guidance_scale=args.guidance_scale, max_num=1000)
    elif any(content in ['erase', 'retain'] for content in content_list):
        dataset = AdaDataset(data_path =f'data/{args.erase_type}.csv', guidance_scale=args.guidance_scale)
    else:
        raise ValueError(f"Unsupported contents for sample2.py: {args.contents}")
